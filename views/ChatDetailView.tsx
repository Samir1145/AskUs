    ...
    <div className={`flex flex-col h-full relative ${isIncognito ? 'bg-gray-900' : 'bg-[#f0f2f5] dark:bg-gray-950'}`}>
        ...
        <div className="flex flex-col flex-1 min-w-0">
            <div className="flex items-center gap-2">
                {isEditingTitle ? (
                    <div className="flex items-center gap-2 w-full max-w-[200px]">
                        ...
                        <button onClick={() => { setTempTitle(chat.title); setIsEditingTitle(false); }} className="text-red-500 hover:bg-red-50 rounded-full p-0.5"><X size={16} /></button>
                    </div>
                ) : (
                    <>
            ...
            <div className="flex items-center gap-1.5 overflow-hidden">
                <span className={`text-xs truncate ${isIncognito ? 'text-gray-400' : 'text-gray-500 dark:text-gray-400'}`}>
                    {chat.contactName}
                    {chat.secondaryAgents && chat.secondaryAgents.length > 0 && ` + ${chat.secondaryAgents.length} experts`}
                </span>
            </div>
        </div>
        </div>
      ...
      <footer className={`p-3 border-t flex items-end space-x-2 ${isIncognito ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100 dark:bg-gray-900 dark:border-gray-800'}`}>
        ...
        <button 
            ...
            className={`w-10 h-10 rounded-full flex items-center justify-center shadow-lg transition-all mb-1 flex-shrink-0 ${
            isListening 
